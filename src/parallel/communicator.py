...
class RingComm:
    def __init__(self, communicator: Comm):
        ...
        self.ring_comm = communicator.Create_cart(
            dims=[1, comm_size], periods=[True, True], reorder=False
        )
    ...
    def exchangeLeft(self, send_buf, recv_buf):
        status = MPI.Status()
        self.ring_comm.Sendrecv(
            send_buf,
            dest=self.left_neighbor_rank,
            sendtag=12,
            recvbuf=recv_buf,
            source=self.right_neighbor_rank,
            recvtag=12,
            status=status,
        )
    ...
    def exchangeRight(self, send_buf, recv_buf):
        ...
        self.ring_comm.Sendrecv(
            send_buf,
            dest=self.right_neighbor_rank,
            sendtag=12,
            recvbuf=recv_buf,
            source=self.left_neighbor_rank,
            recvtag=12,
            status=status,
        )
